...
def test_get_all_posts(authorized_client, test_posts, test_user):
    ...
    assert res.status_code == 200
    user_posts = [post for post in test_posts if post.owner_id == test_user['id']]
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title": "fake title", "content": "fake content"})
    created_post = schemas.Post(**res.json())
    assert created_post.title == "fake title"
    assert created_post.content == "fake content"
